# ==========================================
# Flood Depth Prediction Model without Clusters
# ==========================================
...
class StandardCBAM(Layer):
    ...
    def build(self, input_shape):
        ...
        # Spatial Attention convolutional layer
...
checkpoint_dir = 'checkpoints_no_clusters'
...
def build_model_with_cbam_weighted(X_train_shape, sequence_length, num_stations, return_attention_in_cbam3=False):
    """
    Builds the flood depth prediction model without clusters.
    """
    # ---------------------------
    # 1. Define Inputs
    # ---------------------------
    ...
    # ---------------------------
    # 4. Sum up Attention Outputs
    # ---------------------------
    # Sum over the stations dimension to get a single context vector
    summed_attention = Lambda(lambda x: K.sum(x, axis=1), name='Sum_Attention_Outputs')(attention_outputs)  # Shape: (batch, features)

    # Project the summed attention vector to match ConvLSTM output channels
    num_conv_lstm_channels = int(conv_lstm_output.shape[-1])
    summed_attention_projected = Dense(num_conv_lstm_channels, activation='relu', kernel_initializer='he_normal', name='Dense_Summed_Attention')(summed_attention)  # Shape: (batch, num_conv_lstm_channels)

    # Expand dimensions and tile to spatial dimensions
    height, width = X_train_shape[2], X_train_shape[3]
    expanded_attention = Lambda(lambda x: K.expand_dims(K.expand_dims(x, axis=1), axis=1), name='Expand_Summed_Attention')(summed_attention_projected)  # Shape: (batch, 1, 1, channels)
    tiled_attention = Lambda(lambda x: K.tile(x, [1, height, width, 1]), name='Tile_Attention_To_Spatial')(expanded_attention)  # Shape: (batch, height, width, channels)

    # ---------------------------
    # 5. Modulate Spatial Features with Context
    # ---------------------------

    # Multiply ConvLSTM output with combined context
    modulated_output = Multiply(name='Modulate_Spatial_With_Context')([tiled_attention, conv_lstm_output])
